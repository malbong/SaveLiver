...
public class TurtleSummons : Enemy
{
    ...
    {
        Transform spearRotator = transform.GetChild(2);
        spearRotator.Rotate(0, 0, 30);
    }
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            if (other.name == "TurtleSummoner(Clone)") return; //소환사에게는 죽지않음
            else OnDead(false);
        }
        else if (other.tag == "Spear")
        {
            Transform spear = transform.Find("Spear Rotator");
